 ...
 */
 ...
 /* Severely modified by John Stark to simply grab raw packets and print them out. */
 /* This program is to be run on a vehicle computer that is receiving data packets
 ** from an OBU; these can be forwarded by the OBU running the wsmpforwardserver.c
 ** program.  The wsmpforwardserver program is as provided by Arada, for their
 ** equipment, and this program originated from their code as well. */
...
void dump_message_content(uint8 *buf) {
	printf("\nRaw packet content:\n");
	int elem = 0;
	int max = sizeof(Ind); /* assumes the input buf is pointing to Ind */
	do {
		printf("%04x: ", elem);
		for (int i = 0;  i < 16  && elem < max;  ++i, ++elem) {
			uint8 cur = *(buf + elem);
			if (cur == 0x87  ||  cur == 0x8d) {		/* flag MAP message ID, SPAT message ID */
				printf("%02x*", cur);
			}else {
				printf("%02x ", cur);
			}
...
int main(int argc, char *argv[] )
{
    ...
    while (1)
    {
		...
		/* Added by John */
		dump_message_content((uint8 *)&Ind);
		
		
		
		
		/****** John - don't need this
		if(kml_check == 1){
