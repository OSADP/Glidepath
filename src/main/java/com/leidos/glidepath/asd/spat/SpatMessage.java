 ...
 *
 */
public class SpatMessage implements IAsdMessage {

    private static Logger logger = (Logger) LoggerManager.getLogger(SpatMessage.class);
        ...
        for (Movement movement : movements)   {
            for (LaneSet laneSet : movement.getLaneSets())   {
                if (laneSet.getLane() == lane && laneSet.isStraight())   {
                    ...
                    int thirdCycle = 0;
                    switch(movement.getCurrentStateAsEnum())   {
                        case RED:
                            thirdCycle = GlidepathApplicationContext.getInstance().getAppConfig().getIntValue("spat.green");
                            break;

                        case YELLOW:
                            thirdCycle = GlidepathApplicationContext.getInstance().getAppConfig().getIntValue("spat.red");
                            break;

                        case GREEN:
                            thirdCycle = GlidepathApplicationContext.getInstance().getAppConfig().getIntValue("spat.yellow");
                            break;
                    ...
                    holder.put(DataElementKey.SIGNAL_TIME_TO_THIRD_PHASE, new DoubleDataElement(movement.getMaxTimeRemaining() + thirdCycle));
